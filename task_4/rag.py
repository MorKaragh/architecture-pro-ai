...
def _select_consistent_context(
    ...
    если среди top-k несколько чанков с тем же title, что у первого попадания,
    оставляем только их.

    Если с выбранным title в выдаче только один чанк, сужать контекст нельзя:
    часто первый хит — секция внутри длинной статьи (заголовок совпал с запросом),
    а определение термина лежит в другом документе; иначе модель получает обрывок
    и отвечает «не знаю», хотя в индексе есть релевантные фрагменты.
    """
    ...
    # Сужаем только когда есть явный «кластер» из одного источника (2+ чанка).
