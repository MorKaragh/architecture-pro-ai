...
Скрипт `task_6/update_index.py` инкрементально обновляет уже существующую базу ChromaDB из `task_3/chroma_db/` (коллекция `knowledge_base`), ориентируясь на `sha256` файлов.
...
- `task_6/update_index.py` — выполняет один проход:
  ...
  - добавляет новые чанки в ChromaDB (`task_3/chroma_db/`, коллекция `knowledge_base`);
...
В `stub`-режиме индекс не пишется в `task_3/chroma_db`, а обновляется отдельная коллекция в `task_6/chroma_db_stub/`.
